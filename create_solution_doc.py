...
toc = [
    ...
    ('שאלה 9: פרזנטציה — נקודות עיקריות', True),
...
#  שאלה 9 — פרזנטציה
# ══════════════════════════════════════════════════════════════════════════════
q_header(doc, 'שאלה 9', 'פרזנטציה — נקודות עיקריות', hex_col='1A376C')
p(doc, 'הצגה חיה בפני הצוות + שאלות ותשובות. הטבלה מפרטת את מבנה הסליידים המוצע.',
  italic=True, size=10, color=MID_GREY, sb=0, sa=6)

slide_t = doc.add_table(rows=1, cols=3)
slide_t.style = 'Table Grid'
for i, h in enumerate(['שקף', 'כותרת', 'תוכן / נקודות עיקריות']):
    cell = slide_t.rows[0].cells[i]
    shade_cell(cell, '1A376C'); set_cell_rtl(cell)
    add_run(cell.paragraphs[0], h, bold=True, size=9, color=WHITE)

slides = [
    ('שקף 1', 'סיכום מנהלים',          'האם אנחנו צומחים בבריאות? 3 ממצאים מרכזיים'),
    ('שקף 2', 'נפח ועסקים',            'פיזור חודשי ואזורי — מגמות ורמת ריכוזיות'),
    ('שקף 3', 'ביצועי משלוח',          'שיעור הצלחה 65.2% | שעות שיא | מפת אזורים'),
    ('שקף 4', 'לוח מחוונים (KPI)',     '5 המדדים + מצב עכשווי + גבולות תקין/מדאיג'),
    ('שקף 5', 'הממצא הכי מדאיג',       '34.8% לא נמסרו — ניתוח הגורמים + אזורים'),
    ('שקף 6', 'המלצות',                'שלושה צעדים מיידיים לשיפור שיעור המסירה'),
]
for idx, (sn, st, content) in enumerate(slides):
    row   = slide_t.add_row()
    shade = 'F0F4FF' if idx % 2 == 0 else 'FFFFFF'
    for j, (val, sx) in enumerate([(sn, 'E8EDF5'), (st, shade), (content, shade)]):
        shade_cell(row.cells[j], sx); set_cell_rtl(row.cells[j])
        add_run(row.cells[j].paragraphs[0], val,
                bold=(j == 0), size=9, color=DARK_BLUE if j == 0 else MID_GREY)

# ── שקופית 1: הממצא החשוב ביותר ─────────────────────────────────────────────
sub(doc, 'שקופית 1 — הממצא החשוב ביותר')

p(doc, '❝ 1 מתוך 3 הזמנות לא מגיעה ללקוח — וזה לא בעיה של ביקוש, זו בעיה של היצע שליחים ❞',
  bold=True, size=13, color=DARK_BLUE, sb=4, sa=8)

answer(doc,
    'שיעור המסירה הכולל עומד על 66.1% — כלומר 33.9% מההזמנות (9,716 הזמנות) '
    'לא הגיעו ללקוח. בממוצע ערך הזמנה של ₪119.44, מדובר על פוטנציאל הכנסה '
    'של כ-₪1.16M שלא מומש ב-18 שבועות בלבד.')
answer_bullet(doc,
    'הנתון הקריטי: בשבת — יום השיא בביקוש — שיעור המסירה הוא 63.1% בלבד (הגרוע ביותר). '
    'בימי ראשון, כשהביקוש נמוך ב-19%, שיעור המסירה עולה ל-67.5%. '
    'זה מוכיח שהבעיה היא לא איכות הלקוחות או המוצר — אלא מחסור בשליחים בשעות השיא.')

doc.add_paragraph().paragraph_format.space_after = Pt(6)

# ── שקופית 2: 3 מספרים תומכים ──────────────────────────────────────────────
sub(doc, 'שקופית 2 — שלושה מספרים שמספרים את הסיפור')

data_box(doc, [
    '66.1%  — שיעור מסירה כולל | כלומר 9,716 הזמנות שלא הגיעו ללקוח',
    '',
    '63.1%  — שיעור מסירה בשבת (יום השיא בנפח, 271 הזמנות/יום)',
    '          vs 67.5% בראשון (יום שקט יותר, 227 הזמנות/יום)',
    '          — שלפי כך, כל 100 הזמנות נוספות = 4.4 מסירות שנאבדות',
    '',
    '48.4%  — שיעור מסירה בשבוע 8–14 אפריל (שפל כלל-תקופה)',
    '          כשמשהו הפריע להיצע השליחים, כמעט מחצית מההזמנות נפלו',
])

doc.add_paragraph().paragraph_format.space_after = Pt(6)

# ── שקופית 3: ממצא מפתיע ────────────────────────────────────────────────────
sub(doc, 'שקופית 3 — ממצא שהפתיע אותי')

answer(doc,
    '57.6% מלקוחות המזומן רשמו כרטיס אשראי במערכת — ובחרו לשלם מזומן בכל זאת.')
answer_bullet(doc,
    'זה אומר שיש 9,190 לקוחות שסומכים על HAAT מספיק כדי לשמור כרטיס, '
    'אבל מעדיפים מזומן מסיבה כלשהי (פרטיות? הרגל? חשש מחיוב?).')
answer_bullet(doc,
    'אלה לא לקוחות שחוששים מהאפליקציה — הם כבר רשומים. '
    'עם קמפיין ממוקד קטן (קאשבק, הנחה בתשלום אשראי ראשון) '
    'ניתן לגרום לחלק גדול מהם לעבור — מה שמשפר מעקב, מדידה ואפשרות לתכנית נאמנות.')

doc.add_paragraph().paragraph_format.space_after = Pt(6)

# ── שקופית 4: מה שלא יכולתי להסביר ─────────────────────────────────────────
sub(doc, 'שקופית 4 — מה שלא הצלחתי להסביר לחלוטין')

answer(doc,
    'שבוע 8–14 אפריל: שיעור המסירה קרס ל-48.4% — אבל נפח ההזמנות דווקא עלה ב-15.4%.')
answer_bullet(doc,
    'הסבר אפשרי: ב-13 אפריל 2024 ישראל חוותה תקיפת טילים ומל"טים מאיראן. '
    'אם נהגים בחרו לא לצאת בסוף השבוע הזה — זה מסביר את הקריסה.')
answer_bullet(doc,
    'אך ללא נתוני פעילות שליחים (כמה שליחים היו online, מאיפה, באיזו שעה) '
    'לא ניתן לאשר זאת. יתכן גם כשל טכני, אירוע תפעולי מקומי, '
    'או שילוב שניהם. הנתון מחייב חקירה נוספת.')

doc.add_paragraph().paragraph_format.space_after = Pt(6)

# ── שקופית 5: המלצה קונקרטית ────────────────────────────────────────────────
sub(doc, 'שקופית 5 — המלצה אחת שניתן ליישם השבוע הבא')

p(doc, 'השקיעו בגיוס שליחים ייעודיים לסוף שבוע באזורי ה-TOP.',
  bold=True, size=12, color=GREEN_DARK, sb=4, sa=6)

data_box(doc, [
    'מה לעשות:',
    '  1. גייסו 20–30 שליחים נוספים ספציפית לשישי-שבת באזורים 1, 4, 10',
    '  2. הציעו להם שיפט בונוס: +10% על כל מסירה בסוף שבוע',
    '  3. הפעילו את המערך כבר השבת הקרובה — יש נתון בייסליין למדידה',
    '',
    'למה דווקא זה:',
    '  • שבת: 271 הזמנות/יום, מסירה 63.1% — פער של 4.4% מהממוצע',
    '  • כל שיפור של 5% בשבת = ~68 הזמנות נוספות שנמסרות',
    '  • בAOV של ₪122.76 = ₪8,348 הכנסה נוספת בשבת אחת',
    '  • החזר השקעה: עלות בונוס לשליחים ~₪2,000 → ROI חיובי מהשבת הראשונה',
])

doc.add_paragraph().paragraph_format.space_after = Pt(6)

# ── שקופית 6: שאלה שלא נשאלה ───────────────────────────────────────────────
sub(doc, 'שקופית 6 — שאלה שהייתי רוצה לשאול (ולא נשאלה)')

p(doc, 'מהו ה-LTV של לקוח חוזר לעומת לקוח חד-פעמי, ומה גורם ללקוח לחזור?',
  bold=True, size=12, color=DARK_BLUE, sb=4, sa=6)

answer(doc,
    'הדאטא מראה 42.3% לקוחות חוזרים — אבל לא יודעים האם הם שווים יותר, '
    'מזמינים לעתים קרובות יותר, או מרוצים יותר.')
answer_bullet(doc,
    'למה זה חשוב: בשוק מסירה, עלות רכישת לקוח חדש גבוהה פי 5–7 מעלות שימור. '
    'אם נדע מה מבדיל לקוח שחוזר (אזור, שעה, עסק, שיטת תשלום, שיעור מסירה שקיבל) — '
    'נוכל לתכנן את כל השרשרת הערכית סביב שימור, לא רק סביב רכישה.')
answer_bullet(doc,
    'השאלה הנגזרת: כמה כדאי להשקיע בוודאי מסירה (driver supply) כדי שלקוח שהזמין '
    'בפעם הראשונה יחזור בפעם השנייה? אם מסירה כושלת = אובדן לקוח, '
    'ה-ROI של שיפור delivery rate גבוה בהרבה ממה שנראה ממבט ראשון.')
insight(doc,
    'תובנת הסיום: HAAT יושבת על מוצר עם ביקוש אמיתי (236 הזמנות/יום ועולה). '
    'האתגר הוא לא לגדול בביקוש — אלא לא לאבד 1 מכל 3 לקוחות בדרך. '
...
#  שמירה
# ══════════════════════════════════════════════════════════════════════════════
out = 'C:/Users/Yosef/PycharmProjects/HAAT_assignment/HAAT_DA_Solution.docx'
doc.save(out)
print(f'Saved: {out}')
